...
| CG1b | P1 | **Generics — monomorphized** (`Brand` resolution) | Layer fully-typed per-instantiation classes on top of CG1a's erased base (`Box(Text)` → `Box_Text` with `get_value() -> String`). Largest remaining feature; CG1a is the unbound floor it falls back to. **Scoped + ready** — full wire-offset map + the 4-pass plan (collect instantiations → resolve binding context → recursive emit with param substitution → dedup) + test matrix in [`docs/CG1B_PLAN.md`](CG1B_PLAN.md). |
...
| CG10 | P2 | **Nested / pointer-element lists** | `List(List(T))`, `List(AnyPointer)`, `List(interface)` — `_list_elem_expr` has no LIST/ANY_POINTER/INTERFACE arm → emits `out[i] = null  # TODO`, and `_list_setter` has no nested-list init. Plain `List(struct)` / `List(scalar)` / `List(Text|Data)` work. Need recursive list element read/write (a list whose elements are themselves list pointers). |
