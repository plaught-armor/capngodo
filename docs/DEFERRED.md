...
| id | pri | item | notes |
|---|---|---|---|
| CG1 | P1 | **Generics** (`Brand` resolution) | Generic structs/fields currently fall through `_flat_of` → `Variant`/`null` TODO. Need to resolve `Brand.Scope`/`Binding` from the CGR and instantiate per binding. Largest remaining feature. |
...
| CG6 | P2 | **Interface / AnyPointer field types** | Emit no-op stubs (`null` getter, `pass` setter). Capability fields decode to a table index at runtime but codegen has no accessor yet. |
