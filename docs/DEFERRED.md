...
| CG9 | P2 | **Const declarations** | `NodeWhich.CONST` nodes are never emitted — a schema-level `const maxFoo :Int32 = 5;` is silently dropped. Emit a class-scoped `const` (or static var for non-`const`-safe types like `Packed*`, C1) on the umbrella class, value from `const_value` via the same `_default_for`-style literal builders. |
