...
class Root extends RefCounted:
	...
	class Builder extends CapnBuilder.StructBuilder:
		...
		func set_scores(value: PackedInt32Array) -> void:
			var lb: CapnBuilder.ListBuilder = self.init_list(2, CapnPointer.ElemSize.FOUR_BYTES, value.size())
			lb.set_int32_array(value)

		func init_child() -> Child.Builder:
			var b: Child.Builder = Child.Builder.new()
			self.fill_struct(3, Child.DATA_WORDS, Child.PTR_WORDS, b)
			return b
